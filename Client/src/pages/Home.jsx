...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchStatistics = async () => {
      try {
        ...
        const response = await fetch("https://localhost:7255/api/statistics");
        if (response.ok) {
          const data = await response.json();
          setStatistics(data);
        }
  ...
  const StatCounters = ({ statistics }) => {
    const [ref, inView] = useInView({ triggerOnce: true, threshold: 0.3 });

    const statsList = [
      { num: statistics.partners, text: 'Scholarship Providers', suffix: '+' },
      { num: statistics.registeredStudents, text: 'Registered Students', suffix: '+' },
      { num: statistics.totalScholarships, text: 'Scholarships Listed', suffix: '+' },
      { num: statistics.approvedApplications, text: 'Approved Applications', suffix: '+' },
    ];

    return statsList.map((item, idx) => (
      <div key={idx} className="col-md-3 col-6" ref={ref}>
        <div className="text-center p-4">
          <h3 className="display-4 fw-bold mb-3">
            {inView ? <CountUp end={item.num} duration={2} separator="," /> : '0'}
            {item.suffix}
          </h3>
        ...
        <div className="container position-relative" style={{ zIndex: 1 }}>
          ...
          <div className="row g-4">
            {isLoadingStats ? (
              ...
              <StatCounters statistics={statistics} />
            )}
          </div>
        </div>
      </section>
